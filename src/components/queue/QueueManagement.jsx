...
function QueueManagement() {
  ...
  const generateToken = async () => {
    try {
      const nextToken =
        queue.length > 0 ? queue[queue.length - 1].tokenNumber + 1 : 1;
  ...
  const startServing = async (id) => {
    try {
      ...
      const waitingTokens = queue.filter((token) => token.status === "serving");

      for (const token of waitingTokens) {
        const tokenRef = doc(db, "queueTokens", token.id);
      ...
      // Start New Serving
      const tokenRef = doc(db, "queueTokens", id);
  ...
  const currentServing = queue.find((token) => token.status === "serving");
  ...
  return (
    <div id="queue" className="bg-white mt-8 p-6 rounded-2xl shadow-md">
      ...
      <div className="mt-8 bg-green-100 border border-green-300 p-6 rounded-2xl">
        <h3 className="text-xl font-semibold text-green-800">Now Serving</h3>
          ...
          <tbody>
            {queue.map((token) => (
              <tr key={token.id} className="border-b">
                <td className="p-3 font-semibold">Token {token.tokenNumber}</td>
