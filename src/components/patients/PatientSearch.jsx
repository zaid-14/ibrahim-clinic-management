...
function PatientSearch() {
  ...
  return (
    <div className="bg-white mt-8 p-6 rounded-2xl shadow-md">
      {/* TITLE */}
      <h2 className="text-2xl font-bold text-gray-800">Search by Name</h2>
      ...
      <div className="mt-8 space-y-4">
        {filteredPatients.length === 0 ? (
          ...
          filteredPatients.map((patient) => (
            <div key={patient.id} className="border rounded-xl p-5 bg-gray-50">
              ...
              <p className="mt-2 text-gray-600">{patient.notes}</p>
            </div>
          ))
        )}
      </div>
